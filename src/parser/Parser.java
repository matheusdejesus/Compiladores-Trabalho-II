...
public class Parser {
	...
	private Stmt assign() {
		...
		switch (op.tag()) {
			...
			case PLUS_ASSIGN:
				move();
				return new Assign(id, new Bin(new Token(Tag.SUM, "+"), id, expr()));
			case MINUS_ASSIGN:
				move();
				return new Assign(id, new Bin(new Token(Tag.SUB, "-"), id, expr()));
			case MUL_ASSIGN:
				move();
				return new Assign(id, new Bin(new Token(Tag.MUL, "*"), id, expr()));
			default:
				error("operador de atribuição inválido");
		}
		return null;
	}
